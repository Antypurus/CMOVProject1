package Common;

public class Constants {

    public static final String SERVER_IP = "10.0.0.5:80";
    public static final String Register_Route = SERVER_IP + "/register";
    public static final String Product_List_Route = SERVER_IP + "/products";
    public static final String Transaction_History_Route = SERVER_IP + "/transactions";
    public static final String Coupon_List_Route = SERVER_IP + "/coupons";
    public static final String Checkout_Route = SERVER_IP + "/checkout";

}
